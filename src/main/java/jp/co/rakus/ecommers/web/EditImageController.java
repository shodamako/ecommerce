...
public class EditImageController {
	...
	public String editItem(@Validated EditItemForm editItemForm, BindingResult result, HttpSession session,
		...
		// ファイル拡張子チェック
		if (!(editItemForm.getImage().getOriginalFilename().endsWith(".jpeg"))
				&& !(editItemForm.getImage().getOriginalFilename().endsWith(".jpg"))) {
		...
		editItemForm = editImageService.execute(id, imagePath);
